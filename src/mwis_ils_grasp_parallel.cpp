    ...
    for (size_t c_idx = 0; c_idx < components.size(); ++c_idx) {
        ...
        while (get_elapsed_time() - comp_start_time < comp_time_limit) {
            ...
            while (true) {
                ...
                if (stuck_counter > 3000) {
                    vector<int> current_members;
                    for (int n : comp) if (curr_team[n]) current_members.push_back(n);
                    
                    int drop_count = max(1, (int)(current_members.size() * 0.05));
                    
                    // Note: Since we are using std::shuffle, we pass our thread-local rng
