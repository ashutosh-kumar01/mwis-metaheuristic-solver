...
// We use 295.0 seconds to gracefully terminate before the strict 5-minute system limit.
...
int main(int argc, char* argv[]) {
    ...
    // --------------------------------------------------------
    // 2. DEEP KERNELIZATION (Global Peeling)
    // --------------------------------------------------------
    queue<int> Q;
    for (int i = 0; i < N; ++i) {
    ...
    // --------------------------------------------------------
    // 4. SEQUENTIAL COMPONENT SOLVER (Tabu-Annealing)
    // --------------------------------------------------------
    ...
    for (size_t c_idx = 0; c_idx < components.size(); ++c_idx) {
        ...
        while (get_elapsed_time() - comp_start_time < comp_time_limit) {
            ...
            // O(1) Tabu Memory Array
            vector<int> tabu_lock(N, 0);

            /*
            If tenure is too short: algo will immediately undo the move it just made, causing an infinite
            loop (e.g., kicking node A out, adding node A back, kicking it out)

             If tenure is too long: algorithm restricts its own freedom too much. It might lock a node that needs
             to be moved to find the true optimal score, artificially preventing itself from exploring good paths.
            */

            int tabu_tenure = max(5, (int)min(50.0, comp.size() * 0.01)); 
            ...
            while (true) {
                ...
                // Tabu Lock Check: If the node was recently moved and the temp is low, skip it.
                // We bypass Tabu if T is high (exploration) or if the move is guaranteed to be amazing.
                if (iter < tabu_lock[u] && T < T0 * 0.5) continue; 
                ...
                if (delta > 0 || exp((double)delta / T) > dist01(rng)) {
                    ...
                    // Apply Tabu Locks to prevent immediate reversal
                    tabu_lock[u] = iter + tabu_tenure;
                    for (int v : rollback_removed) tabu_lock[v] = iter + tabu_tenure;
                    for (int v : rollback_added) tabu_lock[v] = iter + tabu_tenure;
        ...
        // Safely commit best found in this component to the global array
        for (int u : comp) {
